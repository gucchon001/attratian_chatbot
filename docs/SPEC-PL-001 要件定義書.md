...
| MVP-Tool-01 | Confluence検索ツール (Stage 1) | CLIENTTOMOスペース限定で3段階CQL検索戦略を用いた高精度Confluence検索機能。80%関連度を目標とする専用最適化を実施。 |
| MVP-Tool-02 | Jira検索ツール (Stage 2予定) | **Stage 2で実装予定**: 3段階JQL検索戦略によるJiraチケット検索機能。Confluenceとの統合検索結果マージを含む。 |
| MVP-UI-01 | インタラクティブ・チャットUI | Streamlitを用いて、ユーザーがボットと対話するための画面を構築する。会話履歴の表示、テキスト入力、送信機能を含む。 |
| **MVP-UI-02** | **Confluenceフィルター機能 (Stage 1)** | **CLIENTTOMOスペース専用の基本フィルター機能。コンテンツタイプ、作成日範囲での絞り込み対応。Stage 3でJira12パラメータとの統合予定。** |
